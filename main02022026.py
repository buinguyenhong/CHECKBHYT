...
DEFAULT_CONFIG = {
    ...
    "bh": {
        ...
        "listbh_date_col": "Ngày ra"   # <-- theo yêu cầu trước đó
    }
}
...
def yyyymmdd_to_date(s: str) -> datetime.date:
...
def cache_get(tu: str) -> Optional[Tuple[str, pd.DataFrame]]:
    """
    Return (cached_end, df) if exists and readable, else None
    """
...
def classify_case_by_ma_lk(ma_lk: str) -> str:
    s = chuan_hoa_ma_lk(ma_lk)
    return "Ngoại trú" if s.startswith("TN.") else "Nội trú"
...
def load_listbh(path: str, key_col: str, date_col: str) -> pd.DataFrame:
    ...
    # fallback an toàn: nếu config ghi cột không tồn tại mà có "Ngày ra"
...
def sql_exec_sp(conn, sp_name: str, tu: str, den: str) -> pd.DataFrame:
    """
    Chạy SP và lấy SELECT resultset đầu tiên.
    Tránh lỗi: NoneType object is not iterable (cursor.description=None).
    """
    cur = conn.cursor()
...
def normalize_sql_list(df_op: pd.DataFrame, df_ip: pd.DataFrame) -> pd.DataFrame:
    req_op = ["TenBenhNhan", "SoBHYT", "Column4", "SoPhieuThanhToanNgoaiTru"]
    req_ip = ["TenBenhNhan", "SoBHYT", "SoPhieu_BA", "khoadieutri"]
    ...
    return out[["Loại ca", "MA_LK", "Họ tên", "Mã thẻ", "Tên khoa", "Mã y tế"]].copy()
...
def load_hosoloichitiet(path: str) -> pd.DataFrame:
    ...
    out = df[["MA_LK", "MALOI", "MOTALOI"]].copy()
    return out

def merge_error_with_sql(hsloi: pd.DataFrame, sql_list: pd.DataFrame) -> pd.DataFrame:
    base = sql_list[["MA_LK", "Họ tên", "Mã thẻ", "Tên khoa", "Mã y tế"]].copy()
    merged = hsloi.merge(base, on="MA_LK", how="left")
    merged["MA_LK"] = merged["MA_LK"].astype(str).apply(chuan_hoa_ma_lk)

    def fix_ma_y_te(row):
        if str(row["MA_LK"]).startswith("TN."):
            return row.get("Mã y tế", "")
        return ""

    merged["Mã y tế"] = merged.apply(fix_ma_y_te, axis=1)
    merged = merged.drop_duplicates(keep="first")

    cols = ["MA_LK", "Họ tên", "Mã thẻ", "Tên khoa", "Mã y tế", "MALOI", "MOTALOI"]
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Step 2: filter date
        row2 = QHBoxLayout()
        self.de_from = QDateEdit(); self.de_to = QDateEdit()
        self.de_from.setCalendarPopup(True); self.de_to.setCalendarPopup(True)
        today = QDate.currentDate()
        self.de_from.setDate(today); self.de_to.setDate(today)
        self.btn_filter_bh = QPushButton("2) Lọc list bh theo ngày")
        self.btn_filter_bh.clicked.connect(self.on_filter_bh)
        row2.addWidget(QLabel("Từ ngày")); row2.addWidget(self.de_from)
        row2.addWidget(QLabel("Đến ngày")); row2.addWidget(self.de_to)
        ...
        def kpi_card(title_text: str):
            ...
            val = QLabel("0"); val.setObjectName("kpiValue")
        ...
        k.addWidget(c1, 0, 0); k.addWidget(c2, 0, 1)
        k.addWidget(c3, 1, 0); k.addWidget(c4, 1, 1)
    ...
    def build_tab_sql(self):
        ...
        # Date for SP
        self.sql_from = QDateEdit(); self.sql_to = QDateEdit()
        self.sql_from.setCalendarPopup(True); self.sql_to.setCalendarPopup(True)
        today = QDate.currentDate()
        self.sql_from.setDate(today); self.sql_to.setDate(today)
        ...
        r = 0
        g.addWidget(QLabel("Driver"), r, 0); g.addWidget(self.cmb_driver, r, 1)
        g.addWidget(QLabel("Server"), r, 2); g.addWidget(self.ed_server, r, 3); r += 1
        g.addWidget(QLabel("Database"), r, 0); g.addWidget(self.ed_db, r, 1)
        g.addWidget(QLabel("Xác thực"), r, 2); g.addWidget(self.cmb_auth, r, 3); r += 1
        g.addWidget(QLabel("User (SQL Auth)"), r, 0); g.addWidget(self.ed_user, r, 1)
        g.addWidget(QLabel("Password"), r, 2); g.addWidget(self.ed_pass, r, 3); r += 1

        g.addWidget(QLabel("Từ ngày"), r, 0); g.addWidget(self.sql_from, r, 1)
        g.addWidget(QLabel("Đến ngày"), r, 2); g.addWidget(self.sql_to, r, 3); r += 1

        g.addWidget(QLabel("SP Ngoại trú"), r, 0); g.addWidget(self.ed_sp_op, r, 1, 1, 3); r += 1
        g.addWidget(QLabel("SP Nội trú"), r, 0); g.addWidget(self.ed_sp_ip, r, 1, 1, 3); r += 1
        ...
        hint = QLabel(
            ...
            "- Nếu DenNgay nhỏ hơn hoặc TuNgay khác → chạy full (vì sql_list không có cột ngày để cắt).\n"
    ...
    def update_buttons(self):
        ...
        has_compare = not self.df_compare.empty
    ...
    def _run_sp_range(self, tu: str, den: str) -> pd.DataFrame:
        """
        Chạy 2 SP trong range [tu, den], normalize -> sql_list.
        """
        self.read_ui_to_config()
    ...
    def on_run_sp(self):
        """
        ✅ Có cache:
          - Nếu đã cache (tu) và cached_end < den: chỉ chạy phần thiếu rồi ghép.
          - Nếu cached_end == den: dùng cache.
          - Nếu cached_end > den hoặc tu khác: chạy full (không cắt được).
        """
        try:
            ...
            if cached is not None:
                ...
                if cached_end == den:
                    ...
                    # chạy phần thiếu (cached_end+1 -> den)
                    ...
                    # cached_end > den: không thể cắt -> chạy full
            ...
            if used_cache:
                thong_bao(self, "OK (Cache)", "Đã tạo sql_list (có sử dụng cache).")
            else:
                thong_bao(self, "OK", "Đã chạy SP và chuẩn hoá sql_list.")
        except Exception as e:
    ...
    def refresh_compare_table(self):
        ...
        cols = ["Loại ca", "MA_LK", "Họ tên", "Mã thẻ", "Tên khoa", "Mã y tế", "Trạng thái"]
